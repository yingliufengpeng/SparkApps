...
/**
  * 使用Spark Steaming + Spark SQL 来动态计算电商中不同类别中最热门的商品排名，例如手机这个类别下面最热门的三种
  * 手机、电视这个类别下最热门的三种电视，该实例在实际生产环境下具有非常重大的意义
  * 实现技术：Spark Streaming + Spark SQL , 之所以Spark Streaing能够使用ML、sql、graphx等功能就是因为有foreachRDD
  * 和transform等接口，这些接口其实基于RDD进行操作，所以一RDD为及时，就可以使用Spark其他的所有的功能，就像是直接
  * 调用API一样简单
  * 假设说这里这里的数据的格式：user item category ，例如Rocky Samsung Andoid
  */
...
//        System.setProperty("hadoop.home.dir", "E:\\centos6.5\\Big_data\\hadoop-2.6.4\\hadoop-2.6.4" )
         ...
         */
        val conf = new SparkConf().setMaster( "spark://Master:7077" ).setAppName( "OnlineTheTop3ItemForEachCategory2DB" )
        ...
        streamContext.checkpoint( "/root/SparkStreaming/CheckPoint_Data" )
        ...
        val userClickLogsDStream = streamContext.socketTextStream( "Master" , 9999 )
         ...
         */
//        val userClickLogsDStream = lines.flatMap{ line => line.split( " " ) }
        ...
        val categoryUserClickLogsDStream = formattedUserClickLogsDStream.reduceByKeyAndWindow( ( v1 : Int , v2 : Int ) =>
                v1 + v2  , ( v1 : Int , v2 : Int ) => v1 - v2  , Seconds( 60 ) , Seconds( 20 )  )
//        val wordCounts = pairs.reduceByKey( _ + _ )
        ...
        categoryUserClickLogsDStream.foreachRDD { rdd => {
                if( rdd.isEmpty() ){
                    ...
                    val resultDataFrame = hiveContext.sql(
                        " SELECT category,item,click_count FROM ( SELECT " +
                            " category,item,click_count,row_number() " +
                            " OVER (PARTITION BY category ORDER BY click_count DESC) rank FROM categoryItemTable) subquery " +
                            " WHERE rank <= 3 "
                    )
                    //                val resultDataFrame = hiveContext.sql( "SELECT * from categoryItemTable" )
                    val resultRowRDD = resultDataFrame.rdd

                    /**
                      * 需要在Master上的MySQL中的sparkstreaming数据库中，创建一张表categorytop3
                      * create table categorytop3( category varchar( 500 ) , item varchar( 2000 ) , click_count int );
                      */
