...
/**
  ...
  */
object SparkStreamingPollDataFromFlume {
    def main(args: Array[String]) {
         ...
         */
        val conf = new SparkConf().setMaster( "local[4]" ).setAppName( "SparkStreamingPollDataFromFlume" )
        ...
        /*
            ...
            只需要工程需写业务逻辑处理代码
         */
        val streamContext = new StreamingContext( conf , Durations.seconds( 30 ) )
...
//        val lines = streamContext.socketTextStream( "Master" , 9999 )
        val lines = FlumeUtils.createPollingStream( streamContext , "Master" , 9999 )
         ...
         */
        val words = lines.flatMap{ event => { val line = new String( event.event.getBody.array )
            ; line.split( " " ) } }
