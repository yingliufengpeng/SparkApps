...
/**
  ...
  */
object SparkStreamingBroadcastAccumulator {
    def main(args: Array[String]) {

        @volatile var  broadcastList : Broadcast[ Array[ String ] ] = null
        @volatile var  accumulator : Accumulator[ Int ] = null
         ...
         */
        val conf = new SparkConf().setMaster( "local[4]" ).setAppName( "SparkStreamingBroadcastAccumulator" )
        ...
        /**
          ...
          */
        broadcastList = streamContext.sparkContext.broadcast(  Array( "Hadoop" , "Mahout" , "Hive" ) )

        /**
          * 全局计数器，用于通知在线过滤了多少黑名单
          */
        accumulator = streamContext.sparkContext.accumulator( 0 , "OnlineBlacklistCounter" )
        ...
        val lines = streamContext.socketTextStream( "Worker" , 9999 )
        ...
        wordCounts.foreachRDD( rdd => {
            rdd.filter( pairs => {
                if( broadcastList.value.contains( pairs._1 ) )  {
                    accumulator.add( pairs._2 )
                    false
                }
                else
                    true
            ...
            println( "BlackList appeared " + " : " + accumulator.value )
        })
