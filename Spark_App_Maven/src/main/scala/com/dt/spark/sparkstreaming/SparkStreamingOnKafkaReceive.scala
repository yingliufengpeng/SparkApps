...
/**
  ...
  */
object SparkStreamingOnKafkaReceive {
    def main(args: Array[String]) {
         ...
         */
        val conf = new SparkConf().setMaster( "local[8]" ).setAppName( "SparkStreamingOnKafkaReceive" )
        ...
        val lines = KafkaUtils.createStream( streamContext ,
                "Master:2181,Worker:2181" , "MyFirstConsumerGroup" ,
                         Map( "HelloKafkaFromSparkStreaming" -> 4 )   )
         ...
         */
        val words = lines.flatMap{ tuple => tuple._2.split( " " ) }
